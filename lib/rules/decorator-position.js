...
function checkDecorators(context, node, options) {
    ...
    context.report({
      ...
      fix(fixer) {
        ...
        return [
          ...
          fixer.replaceTextRange([decorator.end, node.start - 1], ''),
        ];
      },
    });
  }
  ...
  for (const decoratorConfig of options.onDifferentLines) {
    const config = normalizeConfig(decoratorConfig, INTENT.DIFFERENT_LINE);
    const info = decoratorInfo(node, config);
...
function decoratorInfo(node, config) {
  ...
  const currentPositionMatchesIntent =
    ...
    (options.intent === INTENT.DIFFERENT_LINE && positioning.onDifferentLines);
